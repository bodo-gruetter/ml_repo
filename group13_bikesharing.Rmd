...
If only the adjusted R-Squared is considered at this point, it is shown that the starting.model.1 with a value of 0.72 explains the variability of the data better than the other starting models. Thus we take the starting.model.1 as a starting point for the model development.
...
Now all interactions with weak and medium effect are removed from the model. atemp also shows a medium effect. Since it is present in certain interactions with strong effect, atemp is retained. The adapted R-Squared has only been reduced by 0.0003 points due to the adjustments. The final model now has an adjusted R-Squared of 0.7189, which explains about 72% of the data. The final model now looks as follows:
