...
First two models are initialized. The full model with all predictors, the starting model with all interactions and the starting model without considering the interactions.
...
To find out which of the two starting models fits the data better, the results of the two models are first compared.
...
If only the adjusted R-Squared is considered at this point, it is shown that the starting.model.1 with a value of 0.7093 explains the variability of the data better than the starting.model.2 with a value of 0.6838. Thus we take the starting.model.1 as a starting point for the model development.

If now the p-value of the individual predictors and the interaction relationships in the output of starting.model.1 are considered, it becomes clear that certain attributes have no significant effect on cnt. The interaction relationships weathersit:atemp, weathersit:temp, hr:atemp and mnth:temp are deleted from the model.
...
In the second iteration the interaction relations and predictors with a weak and a medium effect are removed from the model. These are: mnth:hum, season:temp and weathersit:atemp.
...
Now all interactions with weak and medium effect are removed from the model. atemp also shows a medium effect. Since it is present in certain interactions with strong effect, atemp is retained. The adapted R-Squared has only been reduced by 0.0003 points due to the adjustments. The final model now has an adjusted R-Squared of 0.709, which explains about 70% of the data. The final model now looks as follows:
...
### Linear Regression
```{r}
lm.bike.1 <- lm(cnt ~ as.factor(season) + as.factor(yr) + as.factor(mnth) + as.factor(hr) + as.factor(weathersit) + temp + atemp + hum + season:temp + season:atemp + season:hum + yr:hum + mnth:temp + mnth:atemp + mnth:hum + hr:temp + hr:atemp + hr:hum + weathersit:temp + weathersit:atemp + weathersit:hum, data = d.bike)
summary(lm.bike.1)
```
...
## 5. Support Vector Machines (Bodo)

## 6. Neural Networks (Cengiz)
