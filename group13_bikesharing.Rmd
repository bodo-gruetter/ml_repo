...
### Testing the Season
Because ggplot allows elegant data visualization with the same syntax for different types of data models, we decide to use ggplot exclusively for graphical representations as far as possible. Therefore we import ggplot2. If we want to visualize the effect of the *season* on the number of bike rentals it is a good idea to use a box plot.
```{r}
library(ggplot2)
...
First we check whether the four seasons differ significantly from each other. For this we apply a simple linear regression model.
```{r}
