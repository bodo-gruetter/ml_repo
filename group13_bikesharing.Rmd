...
```
performance (MSE)
result: the mse is at 0.273
```{r, warning=FALSE}
mean((yhat.rf-d.bike.test.new$class)^2)
```
With the two masses "increase of MSE in percent" and "inrease in node purity" it can be seen that "hr" is by far the most important variable. 
...
# 7. Conclusion
The purpose of this chapter is to compare all the models that have been used in this work to analyze and predict bike rentals. The comparison takes place in two groups: Regression Problems and Classification Problems. The RMSE is used as comparison value for the regression problems and the accuracy is used for classification problems. To guarantee the fairness of the comparison, a 10-fold cross validation is applied.
...
Let us first compare only the regression models. These include: the linear regression model, the non-linear regression model, the possion regression model, the regression tree, bagging, random forest and boosting.

Random Forest has by far the lowest value. The forecast therefore fits the data very well. However, the value must be viewed critically. The random forest was carried out with class as the y-variable, while in the other models cnt was predicted. If we look at the other models, we see that bagging with cnt as the target variable performed best. The second best models are the non-linear and the linear regression models, which perform slightly better than the poisson model. Far behind, however, are the regression tree and boosting.

As the accuracy values of the classification models, i.e. the Neural Network, the Classification Tree and the SVM, show, the SVM with an accuracy of almost 100% is the best model in our analysis. The Classification Tree still classifies with an accuracy of about 70% while the Neural Network is far behind.

In conclusion, it can be said that several models are suitable for predicting bicycle rentals - either as a numerical value as in the regression models, or as a class as in the classification. The best ones include SVM, bagging, random forest and linear or non-linear regression models.