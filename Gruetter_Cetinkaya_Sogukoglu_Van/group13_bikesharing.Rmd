...
Random Forest has by far the lowest value. The forecast therefore fits the data very well. However, the value must be viewed critically. The random forest was carried out with class as the y-variable, while in the other models cnt was predicted. If we look at the other models, we see that bagging with cnt as the target variable performed best. The second best models are the Poisson and the linear regression model, which perform slightly better than the non-linear model. Far behind, however, are the regression tree and boosting.
